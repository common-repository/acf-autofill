class fieldHandler {
	...
	relationship_field_handler(data) {
		setTimeout(function () {
			const length = jQuery(`#acf-${data.key} .acf-rel-item`).length;
			const randomAmount = Helpers.getRandomInt(length);
			for (let i = 1; i <= randomAmount; i++) {
				jQuery(`#acf-${data.key} .acf-rel-item`).eq(i).trigger('click');
			}
		}, 400);
	}

	// TODO
	relationship_subfield_handler(data) {
		const theFields = acf.getFields({ key: data.key });

		theFields.forEach((field) => {
			setTimeout(function () {
				const length = field.$el.find('.acf-rel-item').length;
				const selected = field.$el.find('.values-list');
				const randomAmount = Helpers.getRandomInt(length);
				for (let i = 1; i <= randomAmount; i++) {
					field.$el.find('.acf-rel-item').eq(i).trigger('click');
				}
			}, 400);
		});
	}
	...
	text_handler(data) {
		...
		switch (name) {
			...
			case 'email':
			case 'e-mail':
				randomText = Helpers.getRandomFromArray(textData.emailaddresses);
				break;
			case 'testimonial':
